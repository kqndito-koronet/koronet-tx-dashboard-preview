...
const TXShared = (() => {
  ...
  const SCORECARD_METRICS = [
    { id: 1,  label: 'Active WS',      target: 155,    fmt: 'int',    href: 'activation.html', tbd: false },
    { id: 2,  label: 'Buy Online',     target: 0.20,   fmt: 'pct',    href: 'activation.html', tbd: false },
    { id: 3,  label: 'Proc GMV',       target: 5e6,    fmt: 'dollar', href: 'activation.html', tbd: false },
    { id: 4,  label: 'Sell Online',    target: 0.10,   fmt: 'pct',    href: 'activation.html', tbd: false },
    { id: 5,  label: 'Pipeline Act.',  target: 0.60,   fmt: 'pct',    href: null,               tbd: true  },
    { id: 6,  label: 'Catalog Online', target: 0.45,   fmt: 'pct',    href: null,               tbd: true  },
    { id: 7,  label: 'Fee Rev/mo',     target: 260000, fmt: 'dollar', href: 'changes.html',    tbd: false },
    { id: 8,  label: 'Full Econ Top20',target: null,   fmt: null,     href: null,               tbd: true  },
  ];

  function _scorecardBadgeClass(value, target) {
    if (value == null || target == null) return 'sc-badge-gray';
    const ratio = value / target;
    if (ratio >= 1.0) return 'sc-badge-green';
    if (ratio >= 0.75) return 'sc-badge-amber';
    return 'sc-badge-red';
  }

  function _scorecardFmt(value, fmt) {
    if (value == null) return 'TBD';
    if (fmt === 'int') return String(Math.round(value));
    if (fmt === 'dollar') return fmtDollar(value);
    if (fmt === 'pct') return (value * 100).toFixed(0) + '%';
    return String(value);
  }

  function _scorecardTargetFmt(target, fmt) {
    if (target == null) return 'Calculated';
    if (fmt === 'int') return String(target);
    if (fmt === 'dollar') return fmtDollar(target);
    if (fmt === 'pct') return (target * 100).toFixed(0) + '%';
    return String(target);
  }

  async function _computeScorecardValues() {
    const vals = {};
    try {
      const [accounts, activationRaw, feesRaw, buyRaw] = await Promise.all([
        loadJSON('data/accounts_v3.json').catch(() => null),
        loadJSON('data/activation_data.json').catch(() => null),
        loadJSON('data/current/fees_monthly.json').catch(() => null),
        loadJSON('data/current/buy_monthly.json').catch(() => null),
      ]);

      // Metric 1: Active WS on platform — Client Wholesalers with sell or buy activity
      if (accounts && accounts.accounts) {
        const accts = accounts.accounts;
        const activeWS = accts.filter(a =>
          a.account_class === 'Client' &&
          a.business_type === 'Wholesaler' &&
          (
            (a.months_active_sell != null && a.months_active_sell > 0) ||
            (a.months_active_buy != null && a.months_active_buy > 0) ||
            (a.sell_ytd != null && a.sell_ytd > 0) ||
            (a.buy_ytd != null && a.buy_ytd > 0)
          )
        );
        // Fallback: use activation_data accounts if accounts_v3 lacks activity fields
        if (activeWS.length === 0 && activationRaw && activationRaw.accounts) {
          const actAccts = activationRaw.accounts.filter(a =>
            a.account_class === 'Client' &&
            a.business_type === 'Wholesaler' &&
            (
              (a.months_active_sell != null && a.months_active_sell > 0) ||
              (a.months_active_buy != null && a.months_active_buy > 0) ||
              (a.buy_ytd != null && a.buy_ytd > 0) ||
              (a.sell_online_ytd != null && a.sell_online_ytd > 0)
            )
          );
          vals[1] = actAccts.length || null;
        } else {
          vals[1] = activeWS.length || null;
        }
      }

      // Metrics 2 & 4: avg digital_buy_share and digital_sell_share (from activation_data summary)
      if (activationRaw) {
        if (activationRaw._meta && activationRaw._meta.summary) {
          const s = activationRaw._meta.summary;
          vals[2] = (s.avg_digital_buy_share != null) ? s.avg_digital_buy_share : null;
          vals[4] = (s.avg_digital_sell_share != null) ? s.avg_digital_sell_share : null;
        } else if (activationRaw.summary) {
          const s = activationRaw.summary;
          vals[2] = (s.avg_digital_buy_share != null) ? s.avg_digital_buy_share : null;
          vals[4] = (s.avg_digital_sell_share != null) ? s.avg_digital_sell_share : null;
        }
      }

      // Metric 3: Procurement monthly GMV — latest month total from buy_monthly
      if (buyRaw && buyRaw.data && buyRaw.data.length > 0) {
        // Find the latest month
        const months = [...new Set(buyRaw.data.map(r => r.month))].sort();
        const latestMonth = months[months.length - 1];
        const latestRows = buyRaw.data.filter(r => r.month === latestMonth);
        const total = latestRows.reduce((sum, r) => sum + (r.buy_gmv || r.gmv || r.amount || 0), 0);
        vals[3] = total > 0 ? total : null;
      }

      // Metric 7: TX fee revenue monthly (ex-Axerrio) — latest month ecom+k2k+api sum
      // Axerrio exclusion: company_id "25489" is "-Sole Farms" (the large Axerrio company)
      // Use latest month rows, fee_channel in [ecom, k2k, api], exclude Axerrio
      if (feesRaw && feesRaw.data && feesRaw.data.length > 0) {
        const AXERRIO_IDS = new Set(['25489']);
        const months = [...new Set(
          feesRaw.data
            .filter(r => r.fee_channel !== 'total')
            .map(r => r.month)
        )].sort();
        const latestMonth = months[months.length - 1];
        const latestRows = feesRaw.data.filter(r =>
          r.month === latestMonth &&
          r.fee_channel !== 'total' &&
          !AXERRIO_IDS.has(String(r.company_id))
        );
        const total = latestRows.reduce((sum, r) => sum + (r.fee_amount || 0), 0);
        vals[7] = total > 0 ? total : null;
      }

    } catch (e) {
      console.warn('Scorecard computation error:', e);
    }
    return vals;
  }

  async function renderScorecard(containerId) {
    const container = document.getElementById(containerId);
    if (!container) return;

    // Show loading state
    container.innerHTML = `<div class="tx-scorecard tx-scorecard-loading">Loading scorecard…</div>`;

    let vals = {};
    try {
      vals = await _computeScorecardValues();
    } catch (e) {
      console.warn('Scorecard load failed:', e);
    }

    const items = SCORECARD_METRICS.map(m => {
      if (m.tbd) {
        const html = `<span class="sc-item">
          <span class="sc-label">[${m.id}] ${m.label}:</span>
          <span class="sc-badge sc-badge-gray">TBD / ${_scorecardTargetFmt(m.target, m.fmt)}</span>
        </span>`;
        return html;
      }

      const raw = vals[m.id];
      const badgeCls = _scorecardBadgeClass(raw, m.target);
      const valStr = raw != null ? _scorecardFmt(raw, m.fmt) : '—';
      const tgtStr = _scorecardTargetFmt(m.target, m.fmt);

      const inner = `<span class="sc-label">[${m.id}] ${m.label}:</span>
        <span class="sc-badge ${badgeCls}">${valStr} / ${tgtStr}</span>`;

      if (m.href) {
        return `<a class="sc-item sc-item-link" href="${m.href}">${inner}</a>`;
      }
      return `<span class="sc-item">${inner}</span>`;
    });

    container.innerHTML = `
      <div class="tx-scorecard">
        <span class="sc-title">SCORECARD</span>
        <span class="sc-sep">·</span>
        ${items.join('<span class="sc-sep">●</span>')}
        <span class="sc-target-label">Target Dec 2026</span>
      </div>
    `;
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .sc-item-link:hover { opacity: 0.75; }
      .sc-label {
        font-size: 10px;
        color: #64748b;
        font-weight: 500;
      ...
      .sc-badge-green { background: #f0fdf4; color: #16a34a; border: 1px solid #bbf7d0; }
      .sc-badge-amber { background: #fffbeb; color: #d97706; border: 1px solid #fde68a; }
      .sc-badge-red   { background: #fef2f2; color: #dc2626; border: 1px solid #fecaca; }
      .sc-badge-gray  { background: #f1f5f9; color: #64748b; border: 1px solid #e2e8f0; }
      .sc-target-label {
        margin-left: auto;
        font-size: 9px;
        color: #cbd5e1;
  ...
  function init(currentTabId, filterContainerId, onFilterChange) {
    ...
    // Scorecard strip is injected per-tab by calling renderScorecard('scorecard-container')
    // after init(). Not auto-injected here to allow each tab to control placement.
